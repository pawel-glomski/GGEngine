...
DisplaySystem::DisplaySystem(DisplayModule & displayModule, const CollisionSystem& colSys, const InteractionSystem& intSys) : displayModule(displayModule), colSys(colSys), intSys(intSys)
{}
...
{
	if(intSys.shouldDrawCollisionGrid())
		drawCollisionGrid();
	if(intSys.shouldDrawCollisions())
		drawCollisions();
	drawEntities();
}
